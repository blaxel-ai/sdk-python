...
from ...helpers import default_image, default_labels, unique_name

# Check for API keys
has_relace_key = bool(os.environ.get("RELACE_API_KEY"))
has_morph_key = bool(os.environ.get("MORPH_API_KEY"))


@pytest.mark.skipif(not has_relace_key, reason="RELACE_API_KEY not set")
...
@pytest.mark.skipif(not has_morph_key, reason="MORPH_API_KEY not set")
...
@pytest.mark.skipif(has_relace_key or has_morph_key, reason="API keys are set")
